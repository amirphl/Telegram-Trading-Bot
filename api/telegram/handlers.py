    ...
    async def on_new_message(event):
        try:
            ...
            if not channel_config:
                logger.warning("No channel configuration found for chat_id: %s", chat_id)
                return
            ...
            logger.info("Upserted message %s from channel '%s'", event.message.id, channel_config.channel_title)
            ...
            if channel_config.policy == SignalDiscoveryPolicy.SINGLE_MESSAGE:
                ...
                logger.warning("Unknown signal discovery policy: %s", channel_config.policy)
